...
export default function LocalPlayerOverlay() {
  ...
  // PREMIUM FILE SELECTION SCREEN (When no file is active)
  if (!currentSong) {
    return (
      <AnimatePresence>
        <motion.div 
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-[9999] bg-[#000] flex flex-col items-center justify-center p-6 text-center overflow-hidden font-sans"
        >
            ...
            <motion.div 
                ...
                className="relative z-10 max-w-sm w-full space-y-10"
            >
                <div className="relative mx-auto w-32 h-32">
                    ...
                    />
                    <div className="absolute inset-4 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 rounded-3xl backdrop-blur-md flex items-center justify-center border border-white/10">
                        <FileText size={48} className="text-cyan-400 drop-shadow-[0_0_15px_rgba(34,211,238,0.5)]" />
                    </div>
                </div>

                <div className="space-y-3">
                    <h2 className="text-2xl font-black text-white tracking-tight uppercase">Local Hyper Player</h2>
                    <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.3em]">Hardware Accelerated | 16K Ready</p>
                </div>

                <label className="block group cursor-pointer">
                    <input type="file" accept="video/*" className="hidden" onChange={handleFileSelect} />
                    <div className="w-full py-5 bg-white text-black rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-[0_0_30px_rgba(255,255,255,0.2)]">
                        Select Video File
                    </div>
                </label>

                <button 
                    onClick={() => setIsLocalPlayerOpen(false)}
            ...
            <div className="absolute bottom-10 left-10 right-10 flex justify-between items-end border-t border-white/5 pt-6 opacity-30">
                <div className="text-left space-y-1">
                    <p className="text-white text-[8px] font-black tracking-widest uppercase">System Status</p>
                    <p className="text-cyan-400 text-[10px] font-bold uppercase">Ready_to_Link</p>
                </div>
                <div className="text-right space-y-1 font-mono">
                    <p className="text-white text-[8px] font-bold uppercase">Engine_v.2.0.4</p>
                    <p className="text-white/40 text-[7px]">MacFeed_HyperCore_Active</p>
                </div>
            </div>
        </motion.div>
      </AnimatePresence>
