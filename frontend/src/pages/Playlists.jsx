...
export default function Playlists() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-accent selection:text-white overflow-hidden relative">
      {/* ── MESH BACKGROUND ── */}
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-10">
          <div className="relative">
             <div className="absolute -left-6 top-1/2 -translate-y-1/2 w-1 h-12 bg-gradient-to-b from-accent to-purple-600 rounded-full shadow-[0_0_20px_var(--accent-color)]" />
             <h1 className="text-4xl md:text-7xl font-black uppercase italic tracking-tighter leading-[0.8] mb-2">
                <span className="text-white">MACFEED</span><br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent via-purple-500 to-blue-400">LIBRARY</span>
             </h1>
             <p className="text-white/30 text-[8px] md:text-[9px] font-black uppercase tracking-[0.5em] ml-1">
                LOCAL MEDIA & GLOBAL PLAYLISTS
             </p>
          </div>

           <div className="flex bg-white/5 backdrop-blur-3xl p-1 rounded-2xl border border-white/10 shadow-2xl w-full md:w-auto">
              {['Local', 'Playlists', 'Smart'].map((view) => (
                <button 
                  key={view}
                  onClick={() => setActiveView(view)}
                  className={`flex-1 md:flex-none px-4 md:px-8 py-2 md:py-3 rounded-xl md:rounded-2xl text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] transition-all relative overflow-hidden group ${activeView === view ? 'text-black' : 'text-white/40 hover:text-white'}`}
                >
                  {activeView === view && (
