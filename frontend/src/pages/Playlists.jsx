      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-12 relative z-10">
        ...
        <AnimatePresence mode="wait">
          {activeView === 'Local' ? (
            ...
            >
             <div className="relative w-full aspect-[2/1] md:aspect-[32/10] group">
               {/* 
                   THE ULTIMATE MOBILE SOLUTION:
                   The input is VISIBLE to the browser (block) but INVISIBLE to the user (opacity-0).
                   It covers the ENTIRE box. No JS .click() is needed. No labels are needed.
                   This is 100% bypass for mobile security blockers.
               */}
               <input 
                 type="file" 
               ...
               <div 
                 ...
                 onDrop={onDrop}
                 className={`relative w-full h-full bg-white/[0.01] border rounded-[2.5rem] md:rounded-[5rem] flex flex-col items-center justify-center transition-all duration-1000 overflow-hidden shadow-2xl ${isDragging ? 'bg-accent/10 border-accent' : 'border-white/5 group-hover:bg-white/[0.03] group-hover:border-accent/20'}`}
               >
                  {/* Visual elements (All pointer-events-none so tap reaches the input above) */}
